...
async function addVideo (req: express.Request, res: express.Response) {
  ...
  const wallpaperResolution = videoResolutionInfo.videoWidth * videoResolutionInfo.videoHeight
  const denominator = wallpaperResolution > MAX_IMAGE_SIZE ? wallpaperResolution / MAX_IMAGE_SIZE : 1

  const size = {
    width: Math.floor(videoResolutionInfo.videoWidth / Math.sqrt(denominator)),
    height: Math.floor(videoResolutionInfo.videoHeight / Math.sqrt(denominator))
  }
  ...
  setInterval(() => console.log('Vid', video), 5000)
  ...
  if (videoFile.isAudio()) {
    ...
    videoFile.resolution = videoResolutionInfo.videoFileResolution
  }
